...
def create_choropleth(map, df_points, df_evs, ratio_cum, column, color, legend_name):
	# Converting the dataframe to a dictionary to be able to add values to GeoJSON file so they can be displaid with tooltip
...
# COLOR SCHEMES FOR CHOROPLETH MAPS (used in create_choropleth()):
